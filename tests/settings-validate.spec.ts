...
import { CURSOR_PROVIDER, CURSOR_SETTINGS_NAMESPACE } from '../src/client-contract.ts'
...
describe('settings mutate validation', () => {
  it('rejects duplicate catalog model ids and leaves saved and runtime state unchanged', async () => {
    ...
    const before = ctx.settings.describe().find(item => item.ns === CURSOR_SETTINGS_NAMESPACE)
    expect(before).toBeDefined()
    const ids = (await ctx.llm.listModels(CURSOR_PROVIDER)).map(model => model.id)
    expect(ids).toContain('kept-model')

    await expect(ctx.settings.mutate(CURSOR_SETTINGS_NAMESPACE, [
      { op: 'set', path: ['models'], value: [{ id: 'kept-model' }, { id: 'kept-model' }] },
    ])).rejects.toThrow(/duplicate/)

    const after = ctx.settings.describe().find(item => item.ns === CURSOR_SETTINGS_NAMESPACE)
    expect(after?.revision).toBe(before?.revision)
    expect(after?.value).toEqual(before?.value)
    expect(settings.writes).toHaveLength(0)
    expect((await ctx.llm.listModels(CURSOR_PROVIDER)).map(model => model.id)).toEqual(ids)
  })

  it('rejects a cross-field runLifecycle and leaves saved state unchanged', async () => {
    const { ctx, settings } = await boot()
    const before = ctx.settings.describe().find(item => item.ns === CURSOR_SETTINGS_NAMESPACE)
    const ids = (await ctx.llm.listModels(CURSOR_PROVIDER)).map(model => model.id)

    await expect(ctx.settings.mutate(CURSOR_SETTINGS_NAMESPACE, [
      { op: 'set', path: ['runLifecycle'], value: { maxOpenRuns: 2, maxBindings: 1 } },
    ])).rejects.toThrow(/maxBindings/)
