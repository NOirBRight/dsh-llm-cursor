...
import { INVALID_CREDENTIAL_CODE, LlmAdapter, LlmError, ReasoningEffortId, RetryPolicySchema, attributionHeaders, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
function decodeCursorAuthCancelReply(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || value["ok"] !== true || typeof value["cancelled"] !== "boolean") return void 0;
	return {
		ok: true,
		cancelled: value["cancelled"]
	};
}
function decodeCursorSettingsReadReply(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || !Number.isSafeInteger(value["revision"]) || value["revision"] < 0) return void 0;
...
function decodeCursorUsageView(value) {
	...
	for (const entry of windows) {
		if (!isRecord$4(entry)) return void 0;
		const id = entry["id"];
...
function decodeCursorSaveRequest(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || !Array.isArray(value["models"])) return void 0;
...
function decodeCursorSaveResult(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || !Number.isSafeInteger(value["revision"])) return void 0;
...
function familyHasFastSku(familyId) {
	const id = clusterOf(familyId).toLowerCase();
...
*/
function expandCursorDirectoryRows(models) {
	const ids = new Set(models.map((model) => model.id));
	const extra = [];
	for (const model of models) {
		if (isCursorContextRow(model.id)) continue;
		const fastFamily = cursorBaseFamilyId(model.id).endsWith("-fast");
		if (!fastFamily && familyHasFastSku(model.id) && !ids.has(`${model.id}-fast`)) extra.push({
			...model,
			id: `${model.id}-fast`,
			name: `${model.name ?? model.id} Fast`
		});
		if (familyHasExtendedContext(model.id, model.name ?? "") && !ids.has(`${model.id}-1m`)) extra.push({
			...model,
			id: `${model.id}-1m`,
		...
		if (!fastFamily && familyHasFastSku(model.id) && familyHasExtendedContext(model.id, model.name ?? "") && !ids.has(`${model.id}-fast-1m`)) extra.push({
			...model,
			id: `${model.id}-fast-1m`,
...
function findCatalogModel(catalog, id) {
	return catalog.find((model) => model.id === id) ?? catalog.find((model) => model.variants?.some((variant) => variant.wireId === id)) ?? catalog.find((model) => model.id === splitCursorWireId(id).family) ?? catalog.find((model) => model.id === cursorBaseFamilyId(id)) ?? catalog.find((model) => {
...
function resolveCursorWireId(model, effort, requestedId = model.id) {
	...
	const withFast = (wireId) => {
		if (!wantedFast || splitCursorWireId(wireId).fast || wireId.endsWith("-fast")) return wireId;
		return `${wireId}-fast`;
	};
	if (variants === void 0 || variants.length === 0) return withFast(fallback);
	...
	return withFast(cursorBaseFamilyId((matching.find((variant) => wireHasThinking(variant.wireId)) ?? matching[0])?.wireId ?? variants[0]?.wireId ?? fallback));
}
function variantMaxMode(model, _effort, requestedId = model.id) {
...
*/
const ShellStreamSchema = /*@__PURE__*/ messageDesc(file_agent, 395);
/**
...
*/
const ShellAllowlistPrecheckResultSchema = /*@__PURE__*/ messageDesc(file_agent, 536);
/**
...
*/
const McpAllowlistPrecheckResultSchema = /*@__PURE__*/ messageDesc(file_agent, 538);
/**
...
*/
const WebFetchAllowlistPrecheckResultSchema = /*@__PURE__*/ messageDesc(file_agent, 540);
/**
...
function parseCursorUsageSummary(payload) {
	if (!isRecord$2(payload) || !isRecord$2(payload["individualUsage"])) return [];
	const individual = payload["individualUsage"];
	const windows = [];
	const plan = isRecord$2(individual["plan"]) ? individual["plan"] : void 0;
	const overall = isRecord$2(individual["overall"]) ? individual["overall"] : void 0;
	const onDemand = isRecord$2(individual["onDemand"]) ? individual["onDemand"] : void 0;
...
function parseCursorAuthMeEmail(payload) {
	...
	if (isRecord$2(user) && typeof user["email"] === "string" && user["email"].length > 0) return user["email"];
}
...
async function readJson(fetchImpl, url, headers, signal) {
	...
	if (!response.ok) {
		...
		if (response.status === 401 || response.status === 403) throw new LlmError(`Cursor usage read failed: ${String(response.status)}`, INVALID_CREDENTIAL_CODE);
		throw new Error(`Cursor usage read failed: ${String(response.status)}`);
	}
	return await response.json();
}
const USAGE_CACHE_MS = 5e3;
const OPTIONAL_404_CACHE_MS = 42e4;
const OPTIONAL_USAGE_REQUEST_TIMEOUT_MS = 3e3;
const usageCache = /* @__PURE__ */ new Map();
const usageInFlight = /* @__PURE__ */ new Map();
const optional404Until = /* @__PURE__ */ new Map();
function usageCacheKey(request) {
	return [
		createHash("sha256").update(request.accessToken).digest("hex"),
		request.userId ?? "",
		request.usageURL ?? CURSOR_USAGE_URL,
		request.usageSummaryURL ?? "https://cursor.com/api/usage-summary",
		request.authMeURL ?? "https://cursor.com/api/auth/me"
	].join("\0");
}
async function readWithTimeout(fetchImpl, url, headers, parent, timeoutMs) {
	...
	try {
		return await readJson(fetchImpl, url, headers, controller.signal);
	} finally {
		clearTimeout(timer);
...
async function readCursorUsage(request) {
	...
	if (!request.refresh && cached !== void 0 && cached.expiresAt > clock()) return cached.reply;
	...
	try {
		...
		if (usageInFlight.get(key) === work) usageInFlight.delete(key);
	}
}
async function readOptional(fetchImpl, url, headers, parent, key, now) {
	const until = optional404Until.get(key);
	if (until !== void 0 && until > now()) return void 0;
	try {
		return await readWithTimeout(fetchImpl, url, headers, parent, OPTIONAL_USAGE_REQUEST_TIMEOUT_MS);
	} catch (error) {
		if (error instanceof Error && /404/u.test(error.message)) optional404Until.set(key, now() + OPTIONAL_404_CACHE_MS);
		return;
	}
}
async function readCursorUsageUncached(request, cacheKey) {
	const fetchImpl = request.fetch ?? fetch;
	...
	const cookie = request.userId === void 0 ? void 0 : "WorkosCursorSessionToken=" + encodeURIComponent(request.userId + "::" + request.accessToken);
	...
	const optionalKey = cacheKey + "\0optional";
	let summary;
	const summaryPromise = request.userId === void 0 || request.userId.length === 0 ? Promise.resolve(void 0) : readOptional(fetchImpl, request.usageSummaryURL ?? "https://cursor.com/api/usage-summary", sessionHeaders, request.signal, optionalKey + "\0summary", now);
	const mePromise = request.userId === void 0 || request.userId.length === 0 || request.email !== void 0 ? Promise.resolve(void 0) : readOptional(fetchImpl, request.authMeURL ?? "https://cursor.com/api/auth/me", sessionHeaders, request.signal, optionalKey + "\0me", now);
	summaryPromise.then((value) => {
		summary = value;
	});
	mePromise.then(async (value) => {
		const email = parseCursorAuthMeEmail(value);
		if (email !== void 0) await request.onEmail?.(email);
	});
	const authWindows = usefulUsageWindows(parseCursorAuthUsage(await authPromise));
	if (authWindows.length === 0) summary = await summaryPromise;
	else await Promise.resolve();
	const summaryWindows = summary === void 0 ? [] : parseCursorUsageSummary(summary);
	const resetsAt = summary === void 0 ? void 0 : parseCursorBillingReset(summary);
	const windows = usefulUsageWindows(summaryWindows.length > 0 ? summaryWindows : authWindows);
	if (windows.length === 0) {
		const reply = { status: "unsupported" };
		usageCache.set(cacheKey, {
			expiresAt: now() + OPTIONAL_404_CACHE_MS,
			reply
		});
		return reply;
	}
...
const CURSOR_POLL_FIRST_MINUTE_MS = 1e3;
const CURSOR_POLL_AFTER_FIRST_MINUTE_MS = 2e3;
...
async function defaultOpenBrowser(_url) {}
	...
	for (let attempt = 0; attempt < runtime.pollMaxAttempts; attempt++) {
		...
		try {
			...
			if (response.ok) {
				...
				if (!isRecord$1(data)) throw new Error("Poll returned an invalid body.");
...
async function refreshCursorToken(runtime, apiKeyOrRefreshToken, previous) {
	...
	const data = await response.json();
	if (!isRecord$1(data)) throw new Error("Cursor token refresh returned an invalid body.");
	const accessToken = data["accessToken"];
	const refreshToken = data["refreshToken"];
...
function beginCursorAuth(runtime) {
	...
	const attempt = {
		attemptId,
		authorizationUrl: runtime.loginURL + "?" + params.toString(),
		state: "pending",
		controller
	};
	const attempts = runtime.attempts ?? /* @__PURE__ */ new Map();
	runtime.attempts = attempts;
	attempts.set(attemptId, attempt);
	pollCursorAuth(runtime, uuid, verifier, controller.signal).then(async (tokens) => {
		if (attempt.state !== "pending") return;
		const session = await sessionWithAccountEmail(runtime, sessionFromTokens(runtime, tokens.accessToken, tokens.refreshToken, void 0, tokens.email));
...
function cancelAllCursorAuth(runtime) {
	for (const attempt of runtime.attempts?.values() ?? []) if (attempt.state === "pending") {
		attempt.controller?.abort();
		attempt.state = "cancelled";
		attempt.message = "Sign-in was cancelled.";
	}
}
function cancelCursorAuth(runtime, attemptId) {
	const attempt = runtime.attempts?.get(attemptId);
	if (attempt === void 0 || attempt.state !== "pending") return false;
	attempt.controller?.abort();
	attempt.state = "cancelled";
	attempt.message = "Sign-in was cancelled.";
	return true;
...
function isSameTurnUserMessage(message) {
	return message.role === "user" && message.source.kind !== "tool";
}
/**
...
function findActiveUserMessageIndex(messages) {
	...
	if (last === void 0 || !isSameTurnUserMessage(last)) return -1;
	let start = messages.length - 1;
	while (start > 0) {
		const previous = messages[start - 1];
		if (previous === void 0 || !isSameTurnUserMessage(previous)) break;
		start--;
	}
	for (let i = start; i < messages.length; i++) {
		const message = messages[i];
		if (message !== void 0 && isUserTurn(message)) return start;
	}
	return -1;
}
function activeUserMessages(messages, activeUserMessageIndex) {
	if (activeUserMessageIndex < 0) return [];
	const active = [];
	for (let i = activeUserMessageIndex; i < messages.length; i++) {
		const message = messages[i];
		if (message === void 0 || !isSameTurnUserMessage(message)) break;
		active.push(message);
	}
	return active;
}
...
function resolveSystemPrompt(messages, system) {
...
function buildRootPromptMessagesJson(messages, system, blobStore, activeUserMessageIndex, provider, model) {
	const entries = systemPromptJsons(resolveSystemPrompt(messages, system)).map((json) => storeCursorBlob(blobStore, new TextEncoder().encode(json)));
	const pushJson = (obj) => {
...
function buildRunAction(messages, activeUserMessageIndex, blobStore, images) {
	...
	if (active.some(isUserTurn) && (userText.length > 0 || selectedImages.length > 0)) return create(ConversationActionSchema, { action: {
...
function decodeMcpArguments(args) {
	return JSON.stringify(Object.fromEntries(Object.entries(args).map(([name, value]) => [name, toJson(ValueSchema, fromBinary(ValueSchema, value))])));
}
...
var InteractionMapper = class {
	...
	turnEnded = false;
	chunks = [];
	constructor(ignoredMcpCallIds = /* @__PURE__ */ new Set()) {
		this.ignoredMcpCallIds = ignoredMcpCallIds;
	}
	take() {
	...
	usage() {
		if (this.inputTokens === 0 && this.outputTokens === 0) return void 0;
		return {
	...
	completeMcpFromExec(toolCallIdValue, name, args) {
		...
		if (delta.length > 0) this.chunks.push({
	...
	openMcp(envelopeCallId, toolCall) {
		...
		if (existing !== void 0) {
			...
			if (toolCall?.tool.case === "mcpToolCall") existing.toolCallId = toolCall.tool.value.args?.toolCallId || toolCall.toolCallId || existing.toolCallId;
			return;
		}
		this.closeText();
		this.closeReasoning();
		...
		const innerCallId = toolCall?.tool.case === "mcpToolCall" ? toolCall.tool.value.args?.toolCallId || toolCall.toolCallId || envelopeCallId : envelopeCallId;
...
//#endregion
//#region lib/types/run-registry.js
...
const DEFAULT_RUN_LIFECYCLE = Object.freeze({
	parkedRunTtlMs: 9e5,
	bindingIdleTtlMs: 36e5,
	maxOpenRuns: 64,
	maxBindings: 256,
	heartbeatIntervalMs: 5e3,
	heartbeatJitterRatio: .1
});
/** Owns every Run and binding created by one {@link CursorAdapter}. */
var CursorRunRegistry = class {
	...
	*/
	constructor(lifecycle, internals = {}) {
	...
	*/
	openRun(sessionId, factory) {
		...
		if (this.runs.size >= this.lifecycle.maxOpenRuns) throw new LlmError("llm-cursor: local Cursor Run capacity is full", "LOCAL_CAPACITY");
		...
		if (sessionRuns === void 0) {
			sessionRuns = /* @__PURE__ */ new Set();
	...
	*/
	park(run) {
		...
		if (record === void 0 || record.state === "closed") return;
		record.state = "parked";
	...
	*/
	claimParked(sessionId, matches) {
		const sessionRuns = this.runsBySession.get(sessionKeyOf(sessionId));
		if (sessionRuns === void 0) return void 0;
		const selected = [...sessionRuns].filter((run) => run.state === "parked").sort((left, right) => (left.parkedAt ?? 0) - (right.parkedAt ?? 0)).find((run) => matches(run.value));
		if (selected === void 0) return void 0;
		selected.state = "active";
		selected.parkedAt = void 0;
		if (selected.parkTimer !== void 0) clearTimeout(selected.parkTimer);
		selected.parkTimer = void 0;
		return selected;
	}
	/**
	...
	*/
	closeParkedRuns(sessionId, reason) {
		for (const run of [...this.runsBySession.get(sessionKeyOf(sessionId)) ?? []]) if (run.state === "parked") this.closeRun(run, reason);
	}
	/**
	...
	*/
	closeRun(run, reason) {
		...
		if (record === void 0 || record.state === "closed") return;
		record.state = "closed";
		if (record.heartbeatTimer !== void 0) clearTimeout(record.heartbeatTimer);
		if (record.parkTimer !== void 0) clearTimeout(record.parkTimer);
		record.heartbeatTimer = void 0;
		record.parkTimer = void 0;
		record.parkedAt = void 0;
		try {
	...
	* @param reason - content-free lifecycle reason used by diagnostics.
	*/
	closeSessionRuns(sessionId, reason) {
	...
	*/
	reconfigure(lifecycle) {
		if (this.disposed) return;
		if (Object.keys(lifecycle).every((key) => lifecycle[key] === this.lifecycle[key])) return;
		this.lifecycle = lifecycle;
		for (const run of this.runs) {
			if (run.heartbeatTimer !== void 0) clearTimeout(run.heartbeatTimer);
			run.heartbeatTimer = void 0;
			this.scheduleHeartbeat(run);
			if (run.state === "parked") this.scheduleParkExpiry(run);
		}
		for (const binding of this.bindings.values()) if (binding.runs.size === 0) this.scheduleBindingExpiry(binding);
	...
	*/
	...
	bindingRecord(sessionKey) {
		const existing = this.bindings.get(sessionKey);
		if (existing !== void 0) return existing;
		if (this.disposed) throw new LlmError("llm-cursor: adapter is disposed", "LOCAL_CAPACITY");
		this.reconcileBindingCapacity(this.lifecycle.maxBindings - 1);
		if (this.bindings.size >= this.lifecycle.maxBindings) throw new LlmError("llm-cursor: local Cursor binding capacity is full", "LOCAL_CAPACITY");
		const binding = {
			sessionKey,
	...
	recordOf(run) {
		return this.runs.has(run) ? run : void 0;
	}
	markBindingLive(binding) {
		if (binding.idleTimer !== void 0) clearTimeout(binding.idleTimer);
		binding.idleTimer = void 0;
		binding.idleSince = void 0;
	}
	markBindingIdleIfEmpty(binding) {
		if (binding.runs.size > 0 || this.disposed) return;
		binding.idleSince = this.now();
		this.scheduleBindingExpiry(binding);
	}
	scheduleParkExpiry(run) {
		if (run.parkTimer !== void 0) clearTimeout(run.parkTimer);
		const parkedAt = run.parkedAt;
		if (parkedAt === void 0) return;
	...
	scheduleBindingExpiry(binding) {
		if (binding.idleTimer !== void 0) clearTimeout(binding.idleTimer);
		const idleSince = binding.idleSince;
		if (idleSince === void 0 || binding.runs.size > 0) return;
	...
	reconcileRunCapacity(target) {
		const parked = [...this.runs].filter((run) => run.state === "parked").sort((left, right) => (left.parkedAt ?? 0) - (right.parkedAt ?? 0));
		while (this.runs.size > target && parked.length > 0) {
			const oldest = parked.shift();
			if (oldest !== void 0) this.closeRun(oldest, "park-capacity");
		}
	}
	reconcileBindingCapacity(target) {
		const idle = [...this.bindings.values()].filter((binding) => binding.runs.size === 0).sort((left, right) => (left.idleSince ?? 0) - (right.idleSince ?? 0));
		while (this.bindings.size > target && idle.length > 0) {
			const oldest = idle.shift();
			if (oldest !== void 0) this.deleteIdleBinding(oldest.sessionKey, "binding-capacity");
		}
	}
	deleteIdleBinding(sessionKey, reason) {
		const binding = this.bindings.get(sessionKey);
		if (binding === void 0 || binding.runs.size > 0) return;
		if (binding.idleTimer !== void 0) clearTimeout(binding.idleTimer);
		binding.idleTimer = void 0;
...
*/
const DEFAULT_HEARTBEAT_INTERVAL_MS = DEFAULT_RUN_LIFECYCLE.heartbeatIntervalMs;
function report(runtime, message) {
	if (runtime.debug === void 0) return;
	try {
...
function handleServerMessage(parked, message, tools, pending) {
	...
	if (msgCase === "execServerMessage" && message.message.value !== void 0) {
		...
		const work = Promise.resolve().then(() => {
			...
			for (const invocation of pending.slice(pendingBefore)) parked.mapper.completeMcpFromExec(invocation.toolCallId, invocation.name, invocation.arguments);
		});
		parked.pendingWork.push(work);
		return;
	}
...
async function* continueRun(run, registry, options, runtime, pending) {
	...
	try {
		...
		if (isResourceExhausted(error) && !parked.mapper.hasOutputTokens()) registry.rotateBinding(options.sessionId);
...
*/
async function* runCursorTurn(options, runtime, registry) {
	if (options.stop !== void 0 && options.stop.length > 0) report(runtime, "llm-cursor: GenerateOptions.stop is ignored");
	...
	if (existing !== void 0) {
		...
		parked.mapper = new InteractionMapper(new Set(parked.calls.flatMap((call) => [call.envelopeCallId, call.pending.toolCallId])));
		const pending = [];
		let resumed = true;
		try {
			if (parked.closed || parked.stream.closed || parked.stream.destroyed) throw new Error("parked stream is closed");
			for (const pair of pairParkResults(parked, options.messages)) writeMcpResult(parked.stream, pair.call.pending, pair.text, pair.isError);
			parked.calls = [];
		} catch (error) {
			resumed = false;
	...
	try {
		writeAgent(parked.stream, create(AgentClientMessageSchema, { message: {
			case: "runRequest",
			value: buildRunRequest(options, run.binding, model, runtime.images)
		} }));
	} catch (error) {
		registry.closeRun(run, "stream-error");
		throw error;
	}
	yield* continueRun(run, registry, options, runtime, []);
}
//#endregion
...
*/
function narrowCursorEscalationSchemas(options) {
...
var CursorAdapter = class extends LlmAdapter {
	...
	constructor(config) {
		super();
		this.config = config;
		this.registry = new CursorRunRegistry(config.options().runLifecycle, { ...config.debug === void 0 ? {} : { debug: config.debug } });
	}
	...
	*/
	...
	resolveModel(provider, model, _signal) {
		...
		return Promise.resolve({
			...
			id: model,
			provider,
			context: { contextWindow: isCursorMaxRow(model) || variantMaxMode(found, void 0, model) ? CURSOR_MAX_CONTEXT_WINDOW : found.contextWindow ?? (isCursorMaxRow(found.id) ? 1e6 : 2e5) },
	...
	*/
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: (options) => this.streamWith(runtime, narrowCursorEscalationSchemas(options))
		};
	}
	stream(options) {
		const runtime = this.config.options();
...
function resolveRunLifecycle(config) {
	const resolved = {
	...
	for (const field of [
		...
		if (!Number.isFinite(value) || value <= 0 || value > MAX_TIMER_DELAY_MS) throw new Error(`llm-cursor: runLifecycle.${field} must be a positive finite number no greater than ${MAX_TIMER_DELAY_MS}`);
	}
	for (const field of ["maxOpenRuns", "maxBindings"]) if (!Number.isSafeInteger(resolved[field]) || resolved[field] <= 0) throw new Error(`llm-cursor: runLifecycle.${field} must be a positive safe integer`);
	if (resolved.maxBindings < resolved.maxOpenRuns) throw new Error("llm-cursor: runLifecycle.maxBindings must be greater than or equal to maxOpenRuns");
	if (!Number.isFinite(resolved.heartbeatJitterRatio) || resolved.heartbeatJitterRatio < 0 || resolved.heartbeatJitterRatio > .5) throw new Error("llm-cursor: runLifecycle.heartbeatJitterRatio must be between 0 and 0.5");
	const maximumHeartbeatDelay = resolved.heartbeatIntervalMs * (1 + resolved.heartbeatJitterRatio);
	if (maximumHeartbeatDelay > MAX_TIMER_DELAY_MS) throw new Error(`llm-cursor: maximum jittered heartbeat delay must be no greater than ${MAX_TIMER_DELAY_MS}`);
	if (resolved.parkedRunTtlMs <= maximumHeartbeatDelay) throw new Error("llm-cursor: runLifecycle.parkedRunTtlMs must exceed the maximum jittered heartbeat delay");
	return resolved;
}
...
const Config = z.object({
	...
	runLifecycle: z.object({
		...
		heartbeatJitterRatio: z.number().min(0).max(.5).default(DEFAULT_RUN_LIFECYCLE.heartbeatJitterRatio)
...
const CREDENTIAL_CODES = [INVALID_CREDENTIAL_CODE, "MISSING_CREDENTIAL"];
/**
...
function failureCode(error) {
	if (error instanceof LlmError) return CREDENTIAL_CODES.includes(error.code) ? INVALID_CREDENTIAL_CODE : error.code;
...
function createCursorRpcHandler(runtime, options) {
	return async (endpoint, payload, signal) => {
		...
		if (endpoint === "auth/cancel") {
			if (typeof payload !== "object" || payload === null || typeof payload["attemptId"] !== "string") return internalError("invalid Cursor auth cancel request");
			const attemptId = payload["attemptId"];
			return {
				ok: true,
				value: {
					ok: true,
					cancelled: cancelCursorAuth(runtime, attemptId)
				}
			};
		}
		if (endpoint === "auth/status") {
			if (payload !== void 0 && payload !== null && (typeof payload !== "object" || Array.isArray(payload))) return internalError("invalid Cursor auth status request");
			const rawAttemptId = payload !== void 0 && payload !== null ? payload["attemptId"] : void 0;
			if (rawAttemptId !== void 0 && typeof rawAttemptId !== "string") return internalError("invalid Cursor auth status request");
			if (payload !== void 0 && payload !== null && decodeCursorEmptyRequest(payload) === void 0) return internalError("invalid Cursor auth status request");
			const attemptId = rawAttemptId;
			const attempt = attemptId === void 0 ? void 0 : runtime.attempts?.get(attemptId);
			if (attemptId !== void 0 && attempt === void 0) return internalError("stale Cursor auth attempt");
			return {
				ok: true,
				value: {
					...statusFromSession(await ensureFreshSession(runtime)),
					...attempt === void 0 ? {} : {
						attemptId: attempt.attemptId,
		...
		if (endpoint === "models/list") {
			...
			try {
				const session = await ensureFreshSession(runtime);
				if (session === void 0) return failed(INVALID_CREDENTIAL_CODE, "Sign in to fetch Cursor models");
		...
		if (endpoint === "settings/read") {
			if (decodeCursorEmptyRequest(payload) === void 0) return internalError("invalid Cursor settings read request");
			if (options?.readSettings === void 0) return internalError("Cursor settings are unavailable");
			return {
				ok: true,
				value: options.readSettings()
			};
		}
		...
		if (endpoint === "usage/read") {
			const usageRequest = payload === void 0 || payload === null ? {} : payload;
			if (typeof usageRequest !== "object" || Array.isArray(usageRequest) || Object.keys(usageRequest).some((key) => key !== "refresh") || "refresh" in usageRequest && typeof usageRequest.refresh !== "boolean") return internalError("invalid Cursor usage request");
			let secrets = [];
			try {
				const session = await ensureFreshSession(runtime);
				...
				return {
					...
					value: await withUnauthorizedRetry(runtime, session.accessToken, (accessToken) => readCursorUsage({
						...
						...session.email === void 0 ? {} : { email: session.email },
						refresh: usageRequest.refresh === true,
...
function apply(ctx, config) {
	...
	const readSettings = () => {
		...
		if (descriptor === void 0 || settings === void 0) throw new Error("Cursor settings are unavailable");
		return {
			settings,
	...
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.effect(() => connectionCtx.connection.rpc.handle(CURSOR_RPC_CHANNEL, createCursorRpcHandler(runtime, {
			saveCatalog,
			readSettings
