...
var CursorRunRegistry = class {
	...
	*/
	...
	dispose() {
		...
		for (const binding of this.bindings.values()) if (binding.idleTimer !== void 0) clearTimeout(binding.idleTimer);
		this.bindings.clear();
	...
	reconcileBindingCapacity(target) {
		...
		while (this.bindings.size > target && idle.length > 0) {
			...
			if (oldest !== void 0) this.deleteIdleBinding(oldest.sessionKey);
		}
	}
	deleteIdleBinding(sessionKey) {
