...
//#endregion
//#region lib/types/catalog-group.js
/**
...
function clusterOf(family) {
	return family.endsWith("-fast") ? family.slice(0, -5) : family;
}
const BRAND_RANK = {
	cursor: 1,
	openai: 2,
...
function sortGroupedFamilies(grouped, firstIndex, sort) {
	...
	return [...grouped].sort((left, right) => {
		...
		const leftFast = left.id.endsWith("-fast") ? 1 : 0;
		const rightFast = right.id.endsWith("-fast") ? 1 : 0;
...
function findCatalogModel(catalog, id) {
	return catalog.find((model) => model.id === id) ?? catalog.find((model) => model.variants?.some((variant) => variant.wireId === id));
}
...
function resolveCursorWireId(model, effort) {
	...
	if (variants === void 0 || variants.length === 0) return model.id;
	const wanted = asEffort(effort) ?? defaultEffortOf(model);
	return variants.filter((variant) => (variant.effort ?? "medium") === wanted)[0]?.wireId ?? variants[0]?.wireId ?? model.id;
}
function variantMaxMode(model, effort) {
	const wireId = resolveCursorWireId(model, effort);
	const variant = model.variants?.find((entry) => entry.wireId === wireId);
	if (variant !== void 0) {
		if (variant.maxMode !== true) return false;
		if (variant.effort !== void 0) return true;
...
function parseUsableModels(models) {
	...
	for (const entry of models) {
		...
		const name = entry.modelId === "default" ? entry.displayName.length > 0 && entry.displayName !== "default" ? entry.displayName : "Auto" : entry.displayName.length > 0 ? entry.displayName : entry.modelId;
		out.push({
			id: entry.modelId,
			name,
...
async function readCursorUsage(request) {
	...
	if (request.userId !== void 0 && request.userId.length > 0) {
		...
		try {
			summaryWindows = parseCursorUsageSummary(await readJson(fetchImpl, request.usageSummaryURL ?? "https://cursor.com/api/usage-summary", sessionHeaders, request.signal));
		} catch {}
		try {
...
async function* continueRun(parked, options, runtime, pending) {
	...
	try {
		for (;;) {
			...
			if (status === 401 || status === 403) throw new LlmError("llm-cursor: Cursor session was rejected", "AUTH", { status });
...
var CursorAdapter = class extends LlmAdapter {
	...
	resolveModel(provider, model, _signal) {
		...
		const reasoning = efforts.length > 0 ? {
			...
			efforts: FALLBACK_MAX_MODE_EFFORTS,
