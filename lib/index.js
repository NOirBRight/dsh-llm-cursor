...
function sessionFromTokens(runtime, accessToken, refreshToken, previous) {
	...
	return {
		...
		...previous?.email === void 0 ? {} : { email: previous.email },
		...userId === void 0 ? {} : { userId }
	};
}
	...
	for (let i = 0; i < messages.length; i++) {
		...
		if (isUserTurn(msg)) {
			...
			for (const block of msg.content) {
				...
				if (block.type === "tool-call") parts.push({
					type: "tool-call",
					id: block.id,
					name: block.name,
...
function buildConversationTurns(messages, blobStore, activeUserMessageIndex, provider, model) {
	const turns = [];
...
function buildRunAction(messages, activeUserMessageIndex) {
	...
	const images = active?.content.filter((block) => block.type === "image") ?? [];
	if (active !== void 0 && isUserTurn(active) && (userText.length > 0 || images.length > 0)) return create(ConversationActionSchema, { action: {
		case: "userMessageAction",
		value: create(UserMessageActionSchema, { userMessage: create(UserMessageSchema, {
			text: userText,
			messageId: deterministicUuid(`active:${userText}`),
			...images.length > 0 ? { selectedContext: create(SelectedContextSchema, { selectedImages: images.map(() => create(SelectedImageSchema, { uuid: crypto.randomUUID() })) }) } : {}
		}) })
	} });
...
function buildConversationState(messages, system, blobStore, provider, model) {
...
function parseConnectEndStream(data) {
	try {
		...
		if (error) {
			...
			return /* @__PURE__ */ new Error(`Connect error ${code}: ${message}`);
		}
		return null;
	} catch {
...
function transportError(error) {
	...
	return new LlmError(`llm-cursor: HTTP/2 to the Cursor session entry failed (${message}). The chat path requires HTTP/2 to api2.cursor.sh.`, "SERVER");
}
...
function attachConnectReader(stream) {
	...
	const push = (chunk) => {
		...
		if (waiter !== void 0) waiter(chunk);
		else queue.push(chunk);
	};
	const finish = () => {
		ended = true;
		while (waiters.length > 0) waiters.shift()?.(void 0);
	};
...
async function readConnectPayloads(waitChunk, onFrame) {
...
async function connectUnary(options) {
	const opened = openConnectStream(options.origin, options.path, options.headers);
	const onAbort = () => {
		try {
			opened.stream.close(constants.NGHTTP2_CANCEL);
		} catch {}
		try {
	...
	try {
		...
		if (payload === void 0) throw new Error("Empty Connect unary response");
		return payload;
	} catch (error) {
...
function catalogModel(catalog, id) {
	return catalog.find((model) => model.id === id);
}
function maxModeFor(model, effort) {
...
async function* continueRun(parked, options, runtime, pending) {
	...
	try {
		...
		if (!parked.mapper.turnEnded) throw new LlmError("llm-cursor: stream ended before turnEnded", "SERVER");
	} catch (error) {
...
function buildRunRequest(options, binding, model) {
	const built = buildConversationState(options.messages, options.system, binding.blobStore, options.provider, options.model);
...
async function* runCursorTurn(options, runtime) {
	...
	writeAgent(parked.stream, create(AgentClientMessageSchema, { message: {
		case: "runRequest",
		value: buildRunRequest(options, binding, model)
	} }));
...
const CURSOR_DEFAULT_MODEL_MAX_TOKENS = 16384;
...
var CursorAdapter = class extends LlmAdapter {
	...
	resolveModel(provider, model, _signal) {
		const found = this.config.options().models.find((entry) => entry.id === model) ?? CURSOR_CATALOG.find((entry) => entry.id === model);
		...
		return Promise.resolve({
			...
			...found.maxMode === true ? { reasoning: {
				efforts: MAX_MODE_EFFORTS,
...
function catalogFromSettings(models) {
	if (models === void 0 || models.length === 0) return fallbackCursorCatalog();
...
async function readCursorModels(request) {
	...
	try {
		...
		const models = parseUsableModels(fromBinary(GetUsableModelsResponseSchema, response).models);
...
function parseCursorUsageSummary(payload) {
	...
	const windows = [];
	const plan = isRecord(individual["plan"]) ? individual["plan"] : void 0;
	const overall = isRecord(individual["overall"]) ? individual["overall"] : void 0;
	const onDemand = isRecord(individual["onDemand"]) ? individual["onDemand"] : void 0;
	...
	if (onDemand !== void 0) {
		...
		if (used !== void 0) windows.push(windowOf("On-Demand", used, limit));
	}
	return windows;
}
...
async function readCursorUsage(request) {
	...
	if (request.userId !== void 0 && request.userId.length > 0) {
		...
		try {
			await readJson(fetchImpl, request.authMeURL ?? "https://cursor.com/api/auth/me", sessionHeaders, request.signal);
		} catch {}
	}
...
function usageFailure(error, secrets) {
	let message = error instanceof Error && error.message.length > 0 ? error.message : "Cursor usage read failed";
	...
	return async (endpoint, payload, signal) => {
		...
		if (endpoint === "models/list") {
			...
			if (session === void 0) return {
				ok: true,
				value: { models: fallbackCursorCatalog() }
			};
...
function apply(ctx, config) {
	...
	let lastRaw;
	let lastGood;
	let catalog = catalogFromSettings(config.models);
	const options = () => {
		const raw = current();
	...
	const refreshCatalog = async () => {
		const session = await ensureFreshSession(runtime);
		if (session === void 0) return;
		const models = await readCursorModels({ accessToken: session.accessToken });
	...
	ctx.inject(["connection"], (connectionCtx) => {
		connectionCtx.connection.rpc.handle(CURSOR_RPC_CHANNEL, createCursorRpcHandler(runtime, { adoptCatalog }), { authority: "loopback" });
	});
