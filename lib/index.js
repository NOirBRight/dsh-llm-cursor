...
function decodeCursorAuthCancelReply(value) {
	if (!isRecord$3(value) || hasTokenFields(value) || value["ok"] !== true || typeof value["cancelled"] !== "boolean") return void 0;
	return {
		ok: true,
...
function decodeCursorSettingsReadReply(value) {
	if (!isRecord$3(value) || hasTokenFields(value) || !Number.isSafeInteger(value["revision"]) || value["revision"] < 0) return void 0;
	...
	*/
	async prepareCall(provider, model, signal) {
		...
		return {
			...
			stream: (options) => this.streamWith(runtime, options)
		};
	}
	stream(options) {
		const runtime = this.config.options();
